...
export default function FoodCheckerPage() {
  ...
  const checkCompatibility = async () => {
    ...
    try {
      const res = await fetch(
        `/app/food-checker/food-combo?food1=${food1.toLowerCase()}&food2=${food2.toLowerCase()}`,
      );
  ...
  const getDoshaColor = (impact) => {
    ...
    if (impact.includes("increase") || impact.includes("severe"))
      return "text-red-600";
    return "text-gray-600";
  };

  return (
    <div className="min-h-screen bg-gradient-to-br from-cream to-primary-light/30">
            ...
            <div className="flex items-center justify-between mb-4 flex-wrap gap-4">
              <div>
                ...
                <p className="text-gray-text">
                  Discover which food combinations are beneficial or harmful according
            to Ayurveda
                </p>
              </div>
        ...
        <div className="mt-3 grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <div className="card">
              ...
              {result && (
                <div
                  className={`mt-8 p-6 rounded-xl border-l-4 ${getCompatibilityColor(result.compatibility).bg} ${getCompatibilityColor(result.compatibility).border}`}
                >
                  <div className="flex items-center gap-3 mb-4">
                    <span className="text-4xl">
                      {getCompatibilityColor(result.compatibility).icon}
                    </span>
                    <div>
                      <h3
                        className={`text-2xl font-bold capitalize ${getCompatibilityColor(result.compatibility).text}`}
                      >
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <h4 className="font-bold mb-3 text-dark-text">
                        Dosha Impact:
                      </h4>
                      <div className="grid grid-cols-3 gap-3">
                        {Object.entries(result.doshaImpact).map(
                          ([dosha, impact]) => (
                            <div
                              key={dosha}
                              className="bg-white p-3 rounded-lg text-center"
                            >
                              <div className="font-semibold capitalize text-sm mb-1">
                                {dosha}
                              </div>
                              <div
                                className={`text-xs font-medium capitalize ${getDoshaColor(impact)}`}
                              >
                                {impact}
                              </div>
                            </div>
          ...
          <div className="space-y-6">
              ...
              <ul className="space-y-3 text-sm">
                <li className="flex items-start gap-2">
                  ...
                  <span className="text-dark-text">
                    Incompatible foods can create toxins (ama) in the body
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-primary mt-1">•</span>
                  <span className="text-dark-text">
                    Food combining affects digestion and nutrient absorption
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-primary mt-1">•</span>
                  <span className="text-dark-text">
                    Proper combinations enhance agni (digestive fire)
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-primary mt-1">•</span>
                  <span className="text-dark-text">
                    Wait 1-2 hours between incompatible foods
                  </span>
                </li>
              </ul>
            </div>
            ...
            {recentChecks.length > 0 && (
              <div className="card">
                <h3 className="text-lg font-bold mb-4 text-dark-text">
                  Recent Checks
                </h3>
