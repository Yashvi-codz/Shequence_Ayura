...
/**
 * True if any pantry item matches the recipe ingredient (synonyms + substring).
 */
export function pantryItemMatchesIngredient(pantryItems, recipeIngredientText) {
  if (!recipeIngredientText) return false;
  const list = Array.isArray(pantryItems) ? pantryItems : [];
  return list.some((p) => findBestMatch(p, recipeIngredientText));
}

/**
 * Expanded set of normalized strings for pantry matching (each item + synonym variants).
 */
export function buildPantryMatchSet(pantryItems = []) {
  const set = new Set();
  (pantryItems || []).forEach((p) => {
    getIngredientVariations(p).forEach((v) => {
      const n = normalizeIngredient(v);
      if (n) set.add(n);
    });
  });
  return set;
...
/**
 * Match recipe ingredient string against expanded pantry set (synonyms).
 */
export function recipeIngredientInPantrySet(recipeIngredientText, pantrySet) {
  if (!recipeIngredientText || !pantrySet?.size) return false;
  const vars = getIngredientVariations(recipeIngredientText);
  return vars.some((v) => pantrySet.has(normalizeIngredient(v)));
}
