...
function buildPantrySet(pantryItems = []) {
  return buildPantryMatchSet(pantryItems);
}
...
function scoreRecipe({ recipe, pantrySet, dosha, season, goals, mood }) {
  ...
  recipe.ingredients.forEach((ing) => {
    if (recipeIngredientInPantrySet(ing, pantrySet)) matched += 1;
...
export function rankRecipes({
  ...
  const ranked = AYURVEDIC_RECIPES.map((recipe) => {
    ...
    return {
      ...
      matchedIngredients: recipe.ingredients.filter((i) =>
        recipeIngredientInPantrySet(i, pantrySet)
      ),
...
export function generateGroceryList({ weeklyPlan = [], pantryItems = [] } = {}) {
  ...
  weeklyPlan.forEach((d) => {
    ...
    missing.forEach((ing) => {
      ...
      if (!n) return;
      if (recipeIngredientInPantrySet(ing, pantrySet)) return;
